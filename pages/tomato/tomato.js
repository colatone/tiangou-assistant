...
Page({
  ...
  // 阶段结束提示：横屏时先强制转回竖屏，再走原生弹窗（与竖屏一致）
  _showEndModal: function (isFocus, fromBackground) {
    var self = this
    var show = function () {
      if (fromBackground) {
        wx.showModal({
          ...
          showCancel: false,
          success: function () { self._restoreOrientationAuto() }
        })
      } else {
        wx.showModal({
          title: '完成 1 个番茄 🍅',
    ...
    if (this.data.isLandscape && wx.setPageOrientation) {
      try {
        wx.setPageOrientation({ orientation: 'portrait', complete: function () { show() } })
      } catch (e) { show() }
    } else {
      show()
    }
  ...
  onResetBreak: function () {
    this._setBreak('long', BREAK_DEFAULT, 0)
  },
  ...
  onResetFocus: function () {
    this._setFocus(FOCUS_DEFAULT, 0)
  },
